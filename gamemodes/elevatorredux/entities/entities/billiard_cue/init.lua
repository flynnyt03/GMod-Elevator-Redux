...
function ENT:PhysicsCollide(data, physobj)
  ...
  if not ptable or data.HitEntity:GetClass() ~= "billiard_ball" then return end
  ...
  if data.HitEntity.BallType == "CueBall" or ptable.Training then
    ...
    if data.DeltaTime > 0.1 then
      ...
      elseif data.Speed > 5 and data.Speed <= 20 then
        self:EmitSound("billiards/cuehit_01.wav")
      elseif data.Speed > 20 and data.Speed <= 60 then
        self:EmitSound("billiards/cuehit_02.wav")
      elseif data.Speed > 60 and data.Speed <= 100 then
        self:EmitSound("billiards/cuehit_03.wav")
      elseif data.Speed > 100 and data.Speed <= 140 then
...
function ENT:UpdateCue(ball)
  ...
  if self:GetBilliardTable().OpenBreakShot or self:GetBilliardTable().BehindHeadString then
  ...
  if ball ~= nil and ball.BilliardTableID == self.BilliardTableID and ball:IsValid() then
