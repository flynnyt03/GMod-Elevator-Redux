...
hook.Add("InitPostEntity", "Makethesetvs", function()
	if game.GetMap("elevator") then
		local tv = ents.Create( "elevator_mediatv" )
		tv:SetPos( Vector(1832, -1344, 22) )
		tv:SetAngles(Angle(0,-180,0))
		tv:Spawn()

	end

	if game.GetMap("elevator_redux_beta") then
		local tv = ents.Create( "elevator_media_cinema" )
		tv:SetPos( Vector(9108,-1539,-309) )
		tv:SetAngles(Angle(90,180,0))
		tv:Spawn()
	...
	for _, ent in pairs( ents.GetAll() ) do
		...
		if ( IsValid( ent ) && ent:GetClass() == "info_elevator_floor" ) then
			...
			if ( string.find( tostring( floor ), "elevator_custom" ) ) then
				...
				print( "Custom floor: " .. floor .. " was successfully added!", "\n" )
