...
hook.Add("Think", "CBilliardThink", function()
  if not b_FPerson and LocalPlayer():KeyDown(IN_ATTACK) and not LocalPlayer():KeyDown(IN_SPEED) then
    RunConsoleCommand("billiard_strike", b_ShotForce)
  end

  if input.IsKeyDown(KEY_F7) and not b_keyf7_pressed then
...
function BilliardSpectate(ply, pos, angles, fov)
  ...
  if m_ent and m_ent:IsValid() then
    local view = {}
...
function BilliardHUDPaint()
  ...
  for i, t in pairs(PocketedBalls) do
    ...
    if not t or type(t) ~= "table" then continue end
    b_c = 0
  ...
  if b_GameType == BILLIARD_GAMETYPE_9BALL or b_GameType == BILLIARD_GAMETYPE_ROTATION then
    ...
    if b_GameType == BILLIARD_GAMETYPE_9BALL and b_Objective == 9 then
      b_obj = "9-Ball"
    else
      b_obj = string.format("Ball: %d", b_Objective)
    end
  end
...
usermessage.Hook("billiard_toggleHUD", function(um)
  ...
  if not hook.Exist("CBilliardHUD") then return end
