	...
	if(ptable) then
		...
		if(ptable:IsMyTurn(ply) && !ptable.WaitBallsToStop && ptable:GetTableEnt()) then
			...
			if(ptable.FPerson) then
				...
				if n then n = tonumber(n) or 2 end
				...
				if(ply:KeyDown(IN_SPEED) && b_bool || ply:KeyDown(IN_SPEED) && ptable.Training) then
					...
					if(ply:KeyDown(IN_ATTACK2)) then
						...
						if tobool(n) then b_mx = -b_mx end
						n = ply:GetInfo("billiard_cl_cue_invmouse_v")
						if n then b_my = -b_my end
				...
				if(b_cue.CanShoot && ptable.Foul || ptable.OpenBreakShot && b_cue.CanShoot || ptable.BehindHeadString && b_cue.CanShoot) then
