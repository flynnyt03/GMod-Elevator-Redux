ThirdPerson = {}
ThirdPerson.Dist = CreateClientConVar( "PB_setthirdpersondist", 100, true )
...
hook.Add( "CalcView", "PBThirdPerson", function( ply, origin, angles, fov )
	...
	if ply.ThirdPerson || ret || !ply:Alive() then
		...
		// default values
		local scale = ( ply.PlyModelSize or 1 ) * ply:GetModelScale()
...
hook.Add("ShouldDrawLocalPlayer", "PBThirdDrawLocal", function( ply )
...
hook.Add( "PlayerBindPress", "ThirdPersonViewSelfZoomWheel", function( ply, bind, pressed )
	...
	if LocalPlayer().GetActiveWeapon then
		...
		if ( LocalPlayer().IsPlayingBilliards ) || IsValid( weapon ) && weapon:GetClass() == "weapon_physgun" then  
			return false
		end
	...
	if bind == "invprev" && pressed then
		...
		RunConsoleCommand( "PB_setthirdpersondist", math.Clamp( dist, 35, 150 ) )
...
concommand.Add( "PB_thirdperson", function( ply, cmd, args )
